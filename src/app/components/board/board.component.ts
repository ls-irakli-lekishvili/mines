...
export class BoardComponent implements OnInit {
  ...
  // Game
  setUp(): void {
    this.squares = this.squares.map<Isquare>(element => element = {
      pressed: false,
      handPressed: 0,
      image: "assets/img/dots.svg"
    })
    this.starsOpened = 0;
  }
  ...
  clickHandler(index: number): void {
    if (this.betClicked) {
      ...
      if (this.squares[index].pressed !== true) {
        ...
        if (this.gameLogic()) {
          this.squares[index].image = "assets/img/explode.svg";
          ...
          this.squares[index].image = "assets/img/star.svg";
          this.starAmount--;
          this.starsOpened++;
  ...
  finish() {
    this.squares.forEach(element => {
      if (!element.pressed) {
        ...
        if (this.gameLogic()) {
          element.image = "assets/img/bomb.svg";
          this.mines--;
        }
        else {
          element.image = "assets/img/star.svg";
          this.starAmount--;
        }
      }
    })
  ...
  fastAddMoney(num: number) {
    if (!this.lastPressedButton[num]) {
      ...
      for (let key in this.lastPressedButton) {
        this.lastPressedButton[key] = key == num.toString() ? true : false;
      }
    }
    else {
